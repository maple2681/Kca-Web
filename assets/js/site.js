...
async function sendForm(data) {
  ...
  // Nicer email: clear subject, sender's name, cleaner layout, no technical rows
  const who = clean['Your name'] || clean['Parent / guardian name'] || clean['Full name'] || clean['Email'] || 'a visitor';
  const topic = clean['Topic'] || clean['Grade applying for'] || clean['Position of interest'] || clean['I am a…'] || '';
  const mail = { _subject: `${form}: ${who}${topic ? ' · ' + topic : ''}`, _template: 'box', _captcha: 'false', _replyto: data._replyto || clean.Email || undefined };
  const pages = { '/': 'Home', '/contact': 'Contact', '/admissions': 'Admissions', '/teaching': 'Teaching', '/reviews': 'Reviews', '/donate': 'Donate' };
  Object.keys(clean).forEach(k => { if (k !== 'Form' && k !== 'Page') mail[k] = clean[k]; });
  mail['Sent from'] = (pages[location.pathname] || location.pathname) + ' page · ' + new Date().toLocaleString('en-GB', { dateStyle: 'medium', timeStyle: 'short' });
  if (mail._replyto) mail._autoresponse = 'Thank you for contacting Kyiv Christian Academy! We have received your message and will get back to you within two working days. — KCA Office, +38 (097) 359 7467';
  const toMail = fetch('https://formsubmit.co/ajax/' + FORM_EMAIL, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }, body: JSON.stringify(mail) })
