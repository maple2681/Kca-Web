...
async function sendForm(data) {
  const r = await fetch('https://formsubmit.co/ajax/' + FORM_EMAIL, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }, body: JSON.stringify(data) });
  const out = await r.json().catch(() => ({}));
  if (!r.ok || out.success === 'false' || out.success === false) throw new Error(out.message || 'Send failed');
  return out;
}
...
(() => {
  const pages = [
    ['Home', 'index.html', 'Welcome · KCA'], ['About Us', 'about.html', 'Story, mission, values'], ['Mission & Vision', 'about.html#mission', 'About'], ['Our Team', 'team.html', 'Faculty & staff'],
    ['Accreditation', 'about.html#accreditation', 'ACSI · Middle States'], ['Campus & Safety', 'about.html#campus', 'Shelter · security'],
    ['Statement of Faith', 'about.html#faith', 'About'], ['Academics', 'academics.html', 'Programs & curriculum'],
    ['Elementary School', 'academics.html#elementary', 'Grades K–5'], ['Middle School', 'academics.html#middle', 'Grades 6–8'],
    ['High School', 'academics.html#high', 'Grades 9–12'], ['Admissions', 'admissions.html', 'How to apply'],
    ['Tuition', 'admissions.html#tuition', 'Admissions'], ['Apply / Inquiry form', 'admissions.html#apply', 'Admissions'],
    ['Admissions FAQ', 'admissions.html#faq', 'Admissions'], ['Student Life', 'student-life.html', 'Chapel, events, clubs'],
    ['After-School Program', 'student-life.html#after-school', 'K–5 · 3:15–6:30 pm'], ['School Events', 'student-life.html#events', 'Student Life'],
    ['Reviews', 'reviews.html', 'What families say'], ['Teaching Opportunities', 'teaching.html', 'Open positions'],
    ['Donate', 'donate.html', 'Support KCA'], ['Contact Us', 'contact.html', 'Address, phone, email'], ['Schedule a Tour', 'contact.html#visit', 'Visit the campus'],
  ];
  ...
  document.querySelectorAll('form[data-form]').forEach(f => {
    ...
    f.addEventListener('submit', async e => {
      ...
      if (!ok) return;
      const data = { _subject: 'KCA website: ' + (f.dataset.subject || 'Message'), _template: 'table', _captcha: 'false', 'Form': f.dataset.subject || '', 'Page': location.pathname.split('/').pop() || 'index.html' };
