...
(async () => {
  ...
  #kca-admin .bar select, #kca-admin .bar button { height: 36px; border: 0; border-radius: 10px; padding: 0 12px; background: #333331; color: #fff; font-size: 14px; font-weight: 500; cursor: pointer; }
  ...
  ui.innerHTML = `<div class="bar" role="toolbar" aria-label="Website editor">
      <span class="who"><span class="dot"></span>Editing <small>${user.email}</small></span>
  ...
  bar.querySelector('select').onchange = e => { location.href = e.target.value; };
  ...
  const siteCount = { t: {}, i: {} };
  (async () => {
    const docs = await Promise.all(RT.PAGES.map(p => fetch(p, { cache: 'no-store' }).then(r => r.ok ? r.text() : '').catch(() => '')));
